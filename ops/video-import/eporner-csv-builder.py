...
EPORNER_STANDALONE_TS_RE = re.compile(
    r"(?<![a-z0-9])ts(?![a-z0-9])",
    re.IGNORECASE,
)
...
def source_term_is_normalization_safe(value: str) -> bool:
    """Return True only when Laravel/Python normalization parity is guaranteed.

    VideoTaxonomyResolver::normalizeTerm() uses Laravel Str::ascii(), whose
    transliteration of non-ASCII punctuation/symbols can differ from Python's
    unicodedata-based normalization. Source-term sidecars therefore keep only
    plain ASCII terms. Metadata policy evaluation remains unchanged.
    """
    value = html.unescape(value)
...
def build_metadata(video_id: str, timeout: int, retries: int) -> VideoMetadata:
    ...
    title = prepare_source_title(detail.get("title"))
    if not title:
...
def eporner_editorial_reason(value: str) -> str | None:
    """Provider-local guard for the existing Xurvexa TS exclusion.

    The shared policy intentionally treats the short alias ``ts`` as exact-only
    to avoid broad substring false positives. Eporner metadata frequently uses
    ``TS`` as a standalone token inside longer titles/tags, so this provider
    adapter rejects that token without changing the global V8 policy.
    """
    if EPORNER_STANDALONE_TS_RE.search(value or ""):
...
def metadata_policy_reason(metadata: VideoMetadata) -> str | None:
    for field_name, value in (
        ...
        provider_reason = eporner_editorial_reason(value)
        if provider_reason:
            return (
                "Blocked by Eporner provider guard '"
                f"{provider_reason}' in {field_name}."
            )
    ...
    for tag in metadata.tags:
        ...
        provider_reason = eporner_editorial_reason(tag)
        if provider_reason:
            return (
                "Blocked by Eporner provider guard '"
                f"{provider_reason}' in source tag '{tag}'."
            )
...
def metadata_to_source_terms(
    ...
    for tag in metadata.tags:
        if not source_term_is_normalization_safe(tag):
            continue
