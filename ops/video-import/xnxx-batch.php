...
{
    line('Usage:');
    line('  php xnxx-batch.php <category> [minimum-count] [--prepare-only|--import]');
    line('');
    line('Examples:');
    line('  php xnxx-batch.php latina 500 --prepare-only');
    line('  php xnxx-batch.php latina 500 --import');
    line('');
    line('Options:');
...
function exportExistingXnxxIds(string $path): array
{
    $videos = Video::query()
        ->where('video_source', 'xnxx')
        ->get(['slug', 'embed_url']);
    ...
    foreach ($videos as $video) {
        ...
        if (
            $embedUrl !== '' &&
            preg_match('~/embedframe/([a-z0-9_-]+)~i', $embedUrl, $match)
        ) {
        ...
        if ($id === null) {
            ...
            if (
                preg_match('/^xnxx-([a-z0-9_-]+)(?:-|$)/i', $slug, $match)
            ) {
                $id = strtolower($match[1]);
            }
        }
...
function validateBatch(
    ...
    foreach ($videos as $index => $row) {
        ...
        if (trim((string) ($row['video_source'] ?? '')) !== 'xnxx') {
        ...
        if (! preg_match('~/embedframe/([a-z0-9_-]+)~i', $embedUrl, $match)) {
    ...
    if ($overlapIds !== []) {
        abortRun(
            'Existing XNXX overlap detected: ' .
...
$collector = $ops . '/xnxx-url-collector.py';
$builder = $ops . '/xnxx-csv-builder.py';
...
if (! flock($lockHandle, LOCK_EX | LOCK_NB)) {
    abortRun('Another XNXX batch process is already running.');
}
...
$batchName = $preparedMode
    ? 'xnxx-' . $category . '-prepared-import-v8'
    : 'xnxx-' . $category . '-' . $count . '-v8';
...
$excludeFile = $batch . '/existing-xnxx-ids.txt';
...
if ($preparedMode) {
    ...
    $videoCsv = $batch . '/xurvexa-xnxx-' . $category . '-' . $count . '.csv';
    $rejectCsv = $batch . '/xurvexa-xnxx-' . $category . '-' . $count . '-rejected.csv';
    $termsCsv = $batch . '/xurvexa-xnxx-' . $category . '-' . $count . '-source-terms.csv';
}

line('');
line('===== LIVE EXCLUSION EXPORT =====');
$exclusion = exportExistingXnxxIds($excludeFile);
$liveCount = $exclusion['liveCount'];
$excludedIds = $exclusion['ids'];
kv('LIVE_XNXX_COUNT', $liveCount);
kv('EXCLUSION_ID_COUNT', count($excludedIds));

if ($liveCount !== count($excludedIds)) {
    abortRun(
        'Live XNXX count and unique exclusion ID count differ: ' .
...
if ($mode === 'prepare-only') {
    ...
    kv('XNXX_VIDEOS_NOW', Video::query()->where('video_source', 'xnxx')->count());
    kv('VIDEO_SOURCE_TERMS_NOW', DB::table('video_source_terms')->count());
    line('');
    line('BATCH_PREPARATION=PASS');
    line('===== XURVEXA XNXX BATCH COMPLETE =====');
    exit(0);
}
