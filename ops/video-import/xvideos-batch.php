...
{
    line('Usage:');
    line('  php xvideos-batch.php <category> [minimum-count] [--prepare-only|--import]');
    line('');
    line('Examples:');
    line('  php xvideos-batch.php latina 500 --prepare-only');
    line('  php xvideos-batch.php latina 500 --import');
    line('');
    line('Options:');
...
function exportExistingXvideosIds(string $path): array
{
    $videos = Video::query()
        ->where('video_source', 'xvideos')
        ->get(['slug', 'embed_url']);
    ...
    foreach ($videos as $video) {
        ...
        if (
            $embedUrl !== '' &&
            preg_match('~/embedframe/([a-z0-9]+)~i', $embedUrl, $match)
        ) {
        ...
        if ($id === null) {
            ...
            if (
                preg_match('/^xvideos-([a-z0-9]+)(?:-|$)/i', $slug, $match)
            ) {
                $id = strtolower($match[1]);
            }
        }
...
function validateBatch(
    ...
    foreach ($videos as $index => $row) {
        ...
        if (trim((string) ($row['video_source'] ?? '')) !== 'xvideos') {
        ...
        if (! preg_match('~/embedframe/([a-z0-9]+)~i', $embedUrl, $match)) {
    ...
    if ($overlapIds !== []) {
        abortRun(
            'Existing XVideos overlap detected: ' .
...
$collector = $ops . '/xvideos-url-collector.py';
$builder = $ops . '/xvideos-csv-builder.py';
...
$lockPath = $ops . '/.xvideos-batch.lock';
...
if (! flock($lockHandle, LOCK_EX | LOCK_NB)) {
    abortRun('Another XVideos batch process is already running.');
}
...
$excludeFile = $batch . '/existing-xvideos-ids.txt';
...
if ($preparedMode) {
    ...
    $videoCsv = $batch . '/xurvexa-xvideos-' . $category . '-' . $count . '.csv';
    $rejectCsv = $batch . '/xurvexa-xvideos-' . $category . '-' . $count . '-rejected.csv';
    $termsCsv = $batch . '/xurvexa-xvideos-' . $category . '-' . $count . '-source-terms.csv';
}

line('');
line('===== LIVE EXCLUSION EXPORT =====');
$exclusion = exportExistingXvideosIds($excludeFile);
$liveCount = $exclusion['liveCount'];
$excludedIds = $exclusion['ids'];
kv('LIVE_XVIDEOS_COUNT', $liveCount);
kv('EXCLUSION_ID_COUNT', count($excludedIds));

if ($liveCount !== count($excludedIds)) {
    abortRun(
        'Live XVideos count and unique exclusion ID count differ: ' .
...
if ($mode === 'prepare-only') {
    ...
    kv('XVIDEOS_VIDEOS_NOW', Video::query()->where('video_source', 'xvideos')->count());
    kv('VIDEO_SOURCE_TERMS_NOW', DB::table('video_source_terms')->count());
    line('');
    line('BATCH_PREPARATION=PASS');
    line('===== XURVEXA XVVIDEOS BATCH COMPLETE =====');
    exit(0);
}
