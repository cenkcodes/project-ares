...
{
    line('Usage:');
    line('  php redtube-batch.php <category> [minimum-count] [--prepare-only|--import]');
    line('');
    line('Examples:');
    line('  php redtube-batch.php latina 500 --prepare-only');
    line('  php redtube-batch.php latina 500 --import');
    line('');
    line('Options:');
    ...
    line('  --user-id=1');
    line('  --delay=0.30');
    line('  --target=650  (candidate ceiling; all V8-passing videos up to this target are imported)');
    line('  --max-pages=40');
...
): string
{
    ...
    if (! in_array($status, $allowedExitCodes, true)) {
        abortRun(
...
function exportExistingRedtubeIds(string $path): array
{
    $videos = Video::query()
        ->where('video_source', 'redtube')
        ->get(['slug', 'embed_url']);
    ...
    foreach ($videos as $video) {
        ...
        if ($embedUrl !== '') {
            ...
            if (
                ...
                isset($query['id']) &&
                preg_match('/^[0-9]+$/', (string) $query['id'])
            ) {
                $id = (string) $query['id'];
            }
        }
        ...
        if ($id === null) {
            ...
            if (
                preg_match('/^redtube-([0-9]+)$/i', $slug, $match)
            ) {
...
function validateBatch(
    ...
    foreach ($videos as $index => $row) {
        ...
        if (trim((string) ($row['video_source'] ?? '')) !== 'redtube') {
        ...
        $embedParts = parse_url($embedUrl);
        $embedHost = strtolower((string) ($embedParts['host'] ?? ''));
        $embedQuery = [];

        if (isset($embedParts['query'])) {
            parse_str((string) $embedParts['query'], $embedQuery);
        }

        if (
            $embedHost !== 'embed.redtube.com' ||
            ! isset($embedQuery['id']) ||
            ! preg_match('/^[0-9]+$/', (string) $embedQuery['id'])
        ) {
            abortRun('Invalid embed_url for slug [' . $slug . '].');
        }

        $id = (string) $embedQuery['id'];

        if (! preg_match('/^redtube-([0-9]+)$/i', $slug, $slugMatch)) {
            abortRun('Invalid RedTube slug [' . $slug . '].');
        }

        if ((string) $slugMatch[1] !== $id) {
            abortRun(
                'RedTube slug/embed ID mismatch for slug [' . $slug . '].'
            );
        }
    ...
    if ($overlapIds !== []) {
        abortRun(
            'Existing REDTUBE overlap detected: ' .
...
$query = $category === 'big-ass' ? 'big ass' : $category;
$count = $config['count'];
$mode = $config['mode'];
...
$collector = $ops . '/redtube-url-collector.py';
$builder = $ops . '/redtube-csv-builder.py';
...
$lockPath = $ops . '/.redtube-batch.lock';
...
if (! flock($lockHandle, LOCK_EX | LOCK_NB)) {
    abortRun('Another REDTUBE batch process is already running.');
}
...
$batchName = $preparedMode
    ? 'redtube-' . $category . '-prepared-import-v8'
    : 'redtube-' . $category . '-' . $count . '-v8';
...
$excludeFile = $batch . '/existing-redtube-ids.txt';
$candidateFile = $batch . '/' . $category . '-candidate-urls.txt';
...
if ($preparedMode) {
    ...
    $videoCsv = $batch . '/xurvexa-redtube-' . $category . '-' . $count . '.csv';
    $rejectCsv = $batch . '/xurvexa-redtube-' . $category . '-' . $count . '-rejected.csv';
    $termsCsv = $batch . '/xurvexa-redtube-' . $category . '-' . $count . '-source-terms.csv';
}

line('');
line('===== LIVE EXCLUSION EXPORT =====');
$exclusion = exportExistingRedtubeIds($excludeFile);
$liveCount = $exclusion['liveCount'];
$excludedIds = $exclusion['ids'];
kv('LIVE_REDTUBE_COUNT', $liveCount);
kv('EXCLUSION_ID_COUNT', count($excludedIds));

if ($liveCount !== count($excludedIds)) {
    abortRun(
        'Live REDTUBE count and unique exclusion ID count differ: ' .
...
if ($preparedMode) {
    ...
    $collectorStatus = requireLogValue($collectorOutput, 'COLLECT_STATUS');
    $collectedCount = (int) requireLogValue($collectorOutput, 'URLS_COLLECTED');

    kv('COLLECTED_CANDIDATES', $collectedCount);

    if ($collectedCount < $count) {
        abortRun(
            'Collected candidates below minimum. Minimum [' . $count .
            '] got [' . $collectedCount . '].'
        );
    }
    ...
    if ($collectedCount > $target) {
        abortRun(
            'Collected candidates exceed target. Target [' . $target .
            '] got [' . $collectedCount . '].'
        );
    }

    if ($collectorStatus === 'PASS') {
        if ($collectedCount !== $target) {
            abortRun(
                'Collector PASS count mismatch. Target [' . $target .
                '] got [' . $collectedCount . '].'
            );
        }

        kv('COLLECTOR_RESULT', 'FULL_TARGET');
    } elseif (str_starts_with($collectorStatus, 'PARTIAL:')) {
        if ($collectedCount >= $target) {
            abortRun(
                'Collector PARTIAL count is not below target. Target [' .
                $target . '] got [' . $collectedCount . '].'
            );
        }

        kv('COLLECTOR_RESULT', 'PARTIAL_TARGET_ACCEPTED');
    } else {
        abortRun(
            'Unexpected COLLECT_STATUS [' . $collectorStatus . '].'
        );
    }
...
if ($mode === 'prepare-only') {
    ...
    kv('REDTUBE_VIDEOS_NOW', Video::query()->where('video_source', 'redtube')->count());
    kv('VIDEO_SOURCE_TERMS_NOW', DB::table('video_source_terms')->count());
    line('');
    line('BATCH_PREPARATION=PASS');
    line('===== XURVEXA REDTUBE BATCH COMPLETE =====');
    exit(0);
}
